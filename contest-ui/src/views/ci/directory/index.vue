    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['ci:directory:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['ci:directory:export']"
        >导出</el-button>
      </el-col>
    </el-row>
...
<script>
import { listDirectory, getDirectory, delDirectory, addDirectory, updateDirectory, exportDirectory } from "@/api/ci/directory";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除收藏夹编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有收藏夹数据项?', "警告", {
          ...
          return exportDirectory(queryParams);
        }).then(response => {
